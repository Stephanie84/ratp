...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let isloadedinMemory = UserDefaults.standard.bool(forKey: "isPreloaded")
        if !isloadedinMemory {
            loadData()
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "wherMaTrain")
    ...
    func loadData() {
        removeData()
        print("trying to load data core")
        guard let jsonURL = Bundle.main.url(forResource: "metros", withExtension: "json")
            else { return}
        let jsonData = try! Data(contentsOf: jsonURL)
        let metroData = JSON(data: jsonData)
        let jsonArray = metroData["result"].arrayValue
